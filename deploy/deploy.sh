...
gcloud run deploy deckhand-api \
  ...
  --set-env-vars "GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=global,GOOGLE_GENAI_USE_VERTEXAI=true,PUBSUB_TOPIC=$PUBSUB_TOPIC,GCS_BUCKET=$GCS_BUCKET,FRONTEND_ORIGIN=$FRONTEND_ORIGIN,GOOGLE_OAUTH_CLIENT_ID=$OAUTH_CLIENT_ID,ENABLE_TRACING=true" \
...
gcloud run services update deckhand-api --region "$REGION" \
  --update-env-vars "OAUTH_REDIRECT_URI=${API_URL}/auth/google/callback"
...
echo "Done. API: $API_URL   Worker: $WORKER_URL"
